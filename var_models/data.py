...
class Dataloader:
    def __init__(
        ...
        ----------
        returns
            A dataframe of returns where an index is dates and columns are assets.
        window_size
            A size of a sliding window to generate historical values.
        step_size
            A size of a step to slide over the sequense of returns.
        horizon
            A forecast horizon for VaR values.
        first_pred
            A number of a first predicted VaR, should be greater than a window size.
        weights
...
def get_returns(
    ...
    """
    Calculates returns of given prices: r_{t} = (p_{t+1} - p_{t})/p_{t}.
...
def get_logreturns(
    ...
    """
    Calculates log returns of given prices: r_{t} = log(p_{t+1}) - log(p_{t}).
