...
class Dataloader:
    def __init__(self, series: pd.Series, window_size: int, step_size: int, horizon: int, first_pred: int):
        self.series = series
...
def stocks_returns(assets, weights, from_date, to_date):
    ...
    if not (_file_exists("stocks.csv")):
...
def commodities_returns(assets, weights, from_date, to_date):
    ...
    if not (_file_exists("commodities.csv")):
...
def cryptocurrencies_returns(assets, weights, from_date, to_date):
    ...
    if not (_file_exists("cryptocurrencies.csv")):
