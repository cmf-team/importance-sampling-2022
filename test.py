...
class TestData:
    def test_stocks_returns(self):
        assets = ["AAPL"]
        weights = [1.0]
        returns = stocks_returns(
            assets, weights, from_date="09/02/2022", to_date="09/07/2022"
        )
        ...
        assert np.allclose(
            returns, test_returns, atol=0.0001
        ), "First test have returned incorrect result"
        ...
        assert np.allclose(
            returns, test_returns, atol=0.0001
        ), "Second test have returned incorrect result"
        ...
        assert np.allclose(
            returns, test_returns, atol=0.0001
        ), "Third test have returned incorrect result"
    ...
    def test_cryptocurrencies_returns(self):
        ...
        weights = np.ones(9)
        returns = cryptocurrencies_returns(
            assets, weights, from_date="09/02/2022", to_date="09/07/2022"
        )
        test_returns = pd.Series(
...
class TestMetrics:
    def test_quantile_loss(self):
        ...
        var = np.ones(10)
        assert np.isclose(
            quantile_loss(var, target, alpha=0.9), 1.0461, atol=0.0001
        ), "First test have returned incorrect result"
        assert np.isclose(
            quantile_loss(var, target, alpha=0.1), 0.6269, atol=0.0001
        ), "Second test have returned incorrect result"
...
class TestModels:
    def setup_class(self):
        ...
        weights = [0.3, 0.7]
        returns = stocks_returns(
            assets, weights, from_date="09/02/2020", to_date="09/02/2022"
        )
    ...
    def test_historical_simulation(self):
        ...
        assert np.isclose(
            quantile_loss(var, target, alpha), 0.0038, atol=0.0001
        ), "Function quantile_loss have returned incorrect result"
        assert (
            if_test(var, target) > 0.05
        ), "Function if_loss have returned incorrect result"
    ...
    def test_riskmetrics(self):
        ...
        target = np.array(target)
        assert np.isclose(
            quantile_loss(var, target, alpha), 0.0036, atol=0.0001
        ), "Function quantile_loss have returned incorrect result"
        assert (
            pof_test(var, target, alpha) > 0.05
        ), "Function pof_test have returned incorrect result"
        assert (
            if_test(var, target) > 0.05
        ), "Function if_test have returned incorrect result"
