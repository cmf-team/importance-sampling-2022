...
def com_n_crypto(assets, weights, from_date, to_date):
    ...
    from_date = pd.to_datetime(to_date) - pd.DateOffset(1)
    from_date = str(to_date.month) + "/" + str(to_date.day) + "/" + str(to_date.year)
    for asset in assets:
    ...
    for day in range(0, len(price_history)):
        ...
        try:
            ...
            stock_data[pd.to_datetime(date_history[day]).date()] += (arr_of_day_price[day] - arr_of_day_price[day - 1]) / \
...
def stocks_returns(assets, weights, from_date, to_date):
    ...
    for day in range(1, len(price_history)):
        ...
        try:
            # print(f"1 = {date_history[day]}")
            stock_data[date_history[day].date()] += (arr_of_day_price[day] - arr_of_day_price[day - 1]) / arr_of_day_price[
                day - 1]
        except:
            # print(f"2 = {date_history[day]}")
            stock_data[date_history[day].date()] = (arr_of_day_price[day] - arr_of_day_price[day - 1]) / arr_of_day_price[
                day - 1]
    # print(stock_data)
    return stock_data
